...
use crate::simulation::{SimulationRuntime, EventType, SimulationEvent, EventWrapper};
...
impl NetworkOrchestratorPolicy {
    ...
    pub fn coordinate_timeline(
        ...
        if let Some(ref r) = route_opt {
            if r.composite_fidelity < request.fidelity_target && strategy == StrategyType::HighestFidelity {
                route_opt = find_route(network, &request.from_node, &request.to, StrategyType::LowestLatency);
               }
           }

        let route = match route_opt {
            Some(r) => r,
            None => return SimulationResult { success: false, latency_ms: 0.0, final_fidelity: 0.0, execution_path: vec![] },
           };
        ...
        if route.composite_fidelity < request.fidelity_target {
            let max_purified = PurificationEngine::bbpssw_distill(route.composite_fidelity, self.config.physical.baseline_purify_factor);
        ...
        let min_route_latency_ms: f64 = route.path.windows(2)
                .map(|hop| {
                let hop_distance = network.links.iter()
                       .find(|l| (l.from == hop[0] && l.to == hop[1]) || (l.to == hop[0] && l.from == hop[1]))
                       .map(|l| l.distance)
                       .unwrap_or(0.0);
        ...
        while timestamp_ms < request.max_latency_ms && timestamp_ms < self.config.total_time_cutoff_ms {
            for hop in route.path.windows(2) {
                ...
                if link_generated {
                    let base_fidelity = network.links.iter()
                        .find(|l| (l.from == hop[0] && l.to == hop[1]) || (l.to == hop[0] && l.from == hop[1]))
            ...
            let established: HashSet<(&str, &str)> = runtime.link_establishments.iter()
                .map(|(a, b, _)| {
                    let key = if *a < *b { (a.as_str(), b.as_str()) } else { (b.as_str(), a.as_str()) };
                    key
                })
                .collect();
            ...
            if all_hops_done && route.path.len() > 2 {
                ...
                let hop_fidelities: HashMap<(&str, &str), f64> = runtime.link_establishments.iter()
                    .map(|(a, b, f)| {
                        let key = if *a < *b { (a.as_str(), b.as_str()) } else { (b.as_str(), a.as_str()) };
                        (key, *f)
                    })
                ...
                let e2e_fidelity = route.path.windows(2).map(|hop| {
                    let key = if hop[0] < hop[1] {
                        (hop[0].as_str(), hop[1].as_str())
                    } else {
                        (hop[1].as_str(), hop[0].as_str())
                ...
                for _ in 1..route.path.len() - 1 {
                    running_fidelity = PurificationEngine::bbpssw_distill(running_fidelity, self.config.physical.baseline_purify_factor);
        ...
        if is_satisfied { self.telemetry.successful_requests += 1; }
