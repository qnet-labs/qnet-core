use crate::api::request::{QNetFile, QNetNode, QNetLink};
...
pub enum ValidationErrorKind {
    ...
    UnknownNodeReference { node_id: String, link_id: Option<String> },
    SelfLoop { link_id: String },
    InvalidDistance { link_id: String, value: f64 },
    InvalidFidelity { link_id: String, value: f64 },
    InvalidGenerationRate { link_id: String, value: f64 },
    GraphDisconnected { component_count: usize },
}

impl std::fmt::Display for ValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let path_info = self.path.as_ref()
             .map(|p| format!(" [{}]", p))
             .unwrap_or_default();
        ...
        match &self.error_type {
            ...
            ValidationErrorKind::UnknownNodeReference { node_id, link_id } => {
                let link_info = link_id.as_ref()
                     .map(|id| format!(" in link '{}'", id))
                     .unwrap_or_default();
                write!(f, "Unknown node reference '{}'{}{}", node_id, link_info, path_info)
            }
            ...
            ValidationErrorKind::InvalidDistance { link_id, value } => {
                write!(f, "Invalid distance {} km for link '{}' (must be > 0){}", value, link_id, path_info)
            }
            ValidationErrorKind::InvalidFidelity { link_id, value } => {
                write!(f, "Invalid fidelity {} for link '{}' (must be 0 < x <= 1){}", value, link_id, path_info)
            }
            ValidationErrorKind::InvalidGenerationRate { link_id, value } => {
                write!(f, "Invalid generation rate {} Hz for link '{}' (must be > 0){}", value, link_id, path_info)
            }
            ValidationErrorKind::GraphDisconnected { component_count } => {
                write!(f, "Graph is disconnected with {} components{}", component_count, path_info)
            }
        }
    }
}
        ...
        for node in nodes {
            if !seen_ids.insert(&node.id) {
                result.add_error(ValidationError {
                    error_type: ValidationErrorKind::DuplicateNodeId { id: node.id.clone() },
                    message: String::new(),
                    path: None,
                 });
            }
    ...
    pub fn validate_graph_connected(nodes: &[QNetNode], links: &[QNetLink]) -> ValidationResult {
        ...
        for link in links {
            if let (Some(&from_idx), Some(&to_idx)) =
                 (node_indices.get(&link.from.as_str()), node_indices.get(&link.to.as_str()))
             {
